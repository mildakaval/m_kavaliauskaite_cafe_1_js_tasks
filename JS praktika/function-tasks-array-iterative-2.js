...
{
  const averages = students.map(({modules, ...rest}) => ({
    ...rest,
    modules,
    avg: Math.round(
      modules.reduce(
        (result, {credits, marks}) => result +
          (credits * marks.reduce((x, mark) => x + mark)) / marks.length, 0) / modules.reduce((x, mod) => x + mod.credits, 0)),
  }));
...
console.groupCollapsed('2. Atspausdinti visus Informatikos fakulteto studentus');
{
  const result = students.filter(
    ({faculty}) => faculty === "Informatikos fakultetas"
  );
  console.log(result);
}
console.groupEnd();
...
{
  const result = students.filter(
    ({faculty}) => faculty === "Chemijos fakultetas"
  );
  console.log(result);
}
console.groupEnd();
...
{
  const result = students.filter(
    ({faculty}) => faculty === "Elektros ir elektronikos fakultetas"
  );
  console.log(result);
}
console.groupEnd();
...
{
  const studentAvg = students
  .filter(faculty => faculty.faculty === "Informatikos fakultetas")
  .map(({modules, ...rest}) =>({
    ...rest,
    modules,
    avg: Math.round(
    modules.reduce((result, {credits, marks}) =>
      result + credits* marks.reduce((x, mark) => x + mark) / marks.length , 0) / modules.reduce((x, m) => x + m.credits, 0))
  }));

  console.table(studentAvg);
}
console.groupEnd();
...
{
  const studentAvg = students
  .filter(faculty => faculty.faculty === "Chemijos fakultetas")
  .map(({modules, ...rest}) =>({
    ...rest,
    modules,
    avg: Math.round(
...
{
  const studentAvg = students
  .filter(faculty => faculty.faculty === "Elektros ir elektronikos fakultetas")
  .map(({modules, ...rest}) =>({
    ...rest,
    modules,
    avg: Math.round(
...
{
  const list = students.filter(({course}) => course === 1);
  console.table(list);
}
console.groupEnd();
...
{
  const list = students.filter(({course}) => course === 2);
  console.table(list);
}
console.groupEnd();
...
{
  const list = students.filter(({course}) => course === 3);
  console.table(list);
}
console.groupEnd();
...
{
  const list = students.filter(({course}) => course === 4);
  console.table(list);
}
console.groupEnd();
